...
if (username) {
    document.getElementById('username').innerHTML = "Welcome " + username +" "+`<i class="fa-solid fa-hands-clapping"></i>
    `
}
...
function Exist() {
    for (var i = 0; i < cartona.length; i++) {
        if (cartona[i].email.toLowerCase() == signupEmailInput.value.toLowerCase()) {
            return false
        }
    }
}
function signUp() {
    ...
    if (Exist() == false) {
        document.getElementById('demo').innerHTML = '<p class="text-danger m-3 p-3">email already exists</p>'

    } else {
        cartona.push(signUp)
        localStorage.setItem('usersNames', JSON.stringify(cartona))
        document.getElementById('demo').innerHTML = '<p class="text-success m-3 p-3">Success</p>'
...
function login() {
    ...
    if (Empty() == false) {
        document.getElementById('True').innerHTML = '<span class="text-danger m-3">All inputs is required</span>'
        return false
    }
    for (var i = 0; i < cartona.length; i++) {
        if (cartona[i].email.toLowerCase() == email.toLowerCase() && cartona[i].password.toLowerCase() == password.toLowerCase()) {
...
// The location.hostname property can also be set, to navigate to the same URL with a new hostname.
            ...
            document.getElementById('True').innerHTML = '<span class="p-2 text-danger">incorrect email or password</span>'
        }
    }
